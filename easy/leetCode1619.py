...
def trimMean(arr):
    sumArr,lenArr,removeSize , result = sum(arr), len(arr), int( len(arr) * 0.05) ,0
    arr.sort()
    return (sumArr -  sum(arr[0:removeSize]) - sum(arr[lenArr - removeSize: lenArr]))/ (len(arr) - removeSize * 2)
